...
def compute_loss(model, inputs, feature_id, return_outputs=False, **kwargs):
    ...
    patching_vector = inputs["patching_vector"]
    labels = inputs["labels"]
    patched_output_pos = inputs["patched_output_pos"]

    # Inject the patching vector into the input embeddings at [FEATURE] tokens
    ...
    handle = base.get_input_embeddings().register_forward_hook(
        partial(patch_embeds, feature_idx=feature_idx, feature_vector=patching_vector)
    )
    logits = model(input_ids=toks, attention_mask=attention_mask).logits
    handle.remove()

    # Only compute loss on tokens after [PATCHED_OUTPUT]
    pos = (
        torch.arange(labels.shape[1], device=labels.device)
        .unsqueeze(0)
        .expand(*labels.shape)
    )
    ignore = pos < patched_output_pos.unsqueeze(1)
    ...
    if return_outputs:
        return loss, {
            ...
            "labels": labels,
            "patched_output_pos": patched_output_pos,
        }
    return loss


def generate(model, tokenizer, prompt, patching_vector, max_new_tokens=10):
    return base_generate(model, tokenizer, prompt, patching_vector, max_new_tokens)
